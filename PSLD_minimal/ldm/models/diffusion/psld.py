...
class DDIMSampler(object):
    ...
    def ddim_sampling(self, cond, shape,
        ...
        self.save_test_image(measurements, "outputs/psld-ldm-samples-sr/00measurement.png", is_meas=True)
        ...
        for i, step in enumerate(iterator):
            ...
            if (i + 1) % 100 == 0 or i == 0:
                save_path = f"outputs/psld-ldm-samples-sr/{i+1}_step.png"  
                self.save_test_image(img, save_path)
    ...
    def save_test_image(self, img, path, is_meas = False):
        if not is_meas:
    ...
    def decode(self, x_latent, cond, t_start, unconditional_guidance_scale=1.0, unconditional_conditioning=None,
        ...
        for i, step in enumerate(iterator):
            ...
            x_dec, _ = self.p_sample_ddim(x_dec, cond, ts, index=index, use_original_steps=use_original_steps,
                                          unconditional_guidance_scale=unconditional_guidance_scale,
                                          unconditional_conditioning=unconditional_conditioning)
            save_image(x_dec, 'test_image.png')
