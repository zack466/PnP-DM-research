...
def Diffusion_Purified_CSGM_LDM(model, img_gt, total_num_iterations, csgm_num_iterations, device, 
                            cond_method, ddim_init_timestep, ddim_end_timestep, operator, 
                            inverse_problem_type, noise_std, use_weight_decay=False, 
                            weight_decay_lambda=0, mask=None, full_ddim=True, 
                            ddim_num_iterations=20, purification_schedule='linear', 
                            optimizer='Adam', momentum=0, lr=0.1, save_every_main=50, 
                            save_every_sub=1, verbose=False, root_path=None):
    '''
    LDM version of the algorithm that works in latent space
    '''
    if mask != None:
        mask = mask.to(device)
    img_gt = img_gt.to(device)
    ...
    def encode_image(img):
        with torch.no_grad():
            return pipeline.vae.encode(img).latent_dist.sample() * 0.18215

    def decode_image(latents):
        return pipeline.vae.decode(latents / 0.18215).sample

    model = pipeline.unet.to(device).eval()
    tokenizer = pipeline.tokenizer
    text_encoder = pipeline.text_encoder.to(device)

    # Set up text conditioning
    text_prompt = ""
    prompt = [text_prompt]
    ...
    class UNetWrapper(torch.nn.Module):
        def __init__(self, unet, encoder_hidden_states):
            super().__init__()
            self.unet = unet
        ...
        def forward(self, x, timestep, **kwargs):
            ...
            unet_out = self.unet(x, timestep, encoder_hidden_states=self.encoder_hidden_states, **kwargs)
            noise_pred = unet_out.sample
            noise_pred_uncond, noise_pred_text = noise_pred.chunk(2)
            return noise_pred_uncond + 7.5 * (noise_pred_text - noise_pred_uncond)

    model_wrapper = UNetWrapper(model, encoder_hidden_states)
    ...
    # vb0 in latent space
    with torch.no_grad():
        zero_img = torch.zeros_like(img_gt).to(device)
        vb = encode_image(zero_img)
    
    results_list = []
    
    # Initialize purification timesteps
    purification_timesteps = Purification_Schedule(total_num_iterations, ddim_init_timestep, ddim_end_timestep, schedule_type=purification_schedule)
    ...
    for i in range(total_num_iterations):
        ...
        # decode latent to pixel 
        with torch.no_grad():
            x_pixel = decode_image(vb)
        
        vb_prev = vb.clone().detach()
        
        # Step 2: Enforce data consistency in pixel space
        x_pixel_dc, x_list_sub = CSGM_Solver_Pixel_Space(
            measurements, x_pixel, csgm_num_iterations,
            device, use_weight_decay=use_weight_decay,
            weight_decay_lambda=weight_decay_lambda,
            operator=operator, mask=mask, optimizer=optimizer,
            momentum=momentum, lr=lr, save_every=save_every_sub)
        ...
        # back to latent
        with torch.no_grad():
            z = encode_image(x_pixel_dc)
        
        z_noisy = base_diffusion.q_sample(z, ddim_timestep-1)
        
        if ddim_timestep == 1:
            ddim_timestep = ddim_timestep + 1
            
        if full_ddim:
            # Configure DDIM sampler
            ddim_num_iters = min(ddim_timestep, ddim_num_iterations)
            ddim_num_iters = int(ddim_num_iters)
            
            DDIM_Sampler = create_sampler(
                sampler='ddim',
                steps=ddim_timestep,
                noise_schedule='linear',
                model_mean_type='epsilon',
            ...
            # Set up conditioning function
            measurement_cond_fn = cond_method.conditioning
            sample_fn = partial(DDIM_Sampler.p_sample_loop, model=model_wrapper, measurement_cond_fn=measurement_cond_fn)
            ...
            # Run purification
            with autocast():
                vb_purified, _ = sample_fn(x_start=z_noisy, measurement=measurements, record=False, save_root=None)
        else:
            # One-step Tweedie's formula
            sample_fn = partial(base_diffusion.p_sample, model=model_wrapper)
            with autocast():
                out = sample_fn(x=z_noisy, t=torch.tensor(int(ddim_timestep-1)).unsqueeze(0).to(device))
                vb_purified = out['pred_xstart'].detach()
        
        vb = vb_purified
        
        with torch.no_grad():
            x_pixel_output = decode_image(vb)
            
        results_list.extend(x_list_sub)
        results_list.append(x_pixel_output)
        ...
        if i % save_every_main == 0 or i == total_num_iterations - 1:
            if verbose:
                ...
                plt.subplot(142)
                plt.title('Before Purification')
                with torch.no_grad():
                    prev_decoded = decode_image(vb_prev)
                plt.imshow(torch_to_np(normalize_image(prev_decoded)))
                
                plt.subplot(143)
                plt.title('Noisy Latent Decoded')
                with torch.no_grad():
                    noisy_decoded = decode_image(z_noisy)
                plt.imshow(torch_to_np(normalize_image(noisy_decoded)))
                
                plt.subplot(144)
                plt.title('Purified Result')
                plt.imshow(torch_to_np(normalize_image(x_pixel_output)))
                
                fig_name = f'Iter_{i}.png'
                plt.savefig(root_path + fig_name)
    
    return x_pixel_output, results_list
...
def main():
  ...
  for i, img in enumerate(loader):
      ...
      x, x_list_complete = Diffusion_Purified_CSGM_LDM(model, img_gt=img, total_num_iterations=total_num_iterations, 
      ...
      #x_list_complete = x_list_complete[:len(x_list_complete) - 1]
